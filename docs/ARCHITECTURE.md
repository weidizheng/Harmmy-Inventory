...
1. 单品调整在当前商品卡片内展开，只生成该商品的一条操作明细；不再跳转通用库存表单。
2. 批量调整允许用户在多个商品卡片上编辑箱、端、盒差异，浏览器先保存为未提交草稿。
3. 两种入口都显示当前数量、本次变化和提交后结果；单品调整二次确认，批量调整进入统一汇总。
4. `confirm_stock_operation` RPC 在一个数据库事务中锁定余额、校验全部项目、拒绝负库存、写入操作单和前后快照，然后更新余额。
5. 数据库 Trigger 将操作人、时间和 before/after 数据写入 `activity_logs`。
