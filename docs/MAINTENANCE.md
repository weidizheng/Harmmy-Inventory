...
- 查看系统“日志”中是否有异常的大额库存调整或未知操作人；先查看操作单摘要，再展开商品明细核对 before/delta/after；
...
日志首页的数量应以“张操作单”为单位。若一次批量提交在首页出现多条商品记录，应先检查页面是否仍在直接展示 `activity_logs`；普通日志必须从 `stock_operations` 分页，并通过 `stock_operation_items.operation_id` 展开明细。
