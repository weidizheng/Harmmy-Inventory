...
export function AddProductForm({ ips }: Readonly<{ ips: IpOption[] }>) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const customIpName = String(form.get("customIpName") ?? "").trim();
    ...
    if (ipChoice === "__custom__" && !customIpName) { setMessage("请输入自定义 IP 名称。"); return; }
    setSubmitting(true);
    const supabase = createSupabaseBrowserClient();
    let storagePath: string | null = null;
    let productId: string | null = null;
    let createdIpId: string | null = null;
    try {
      ...
      let primaryIpId = ipChoice || null;
      if (ipChoice === "__custom__") {
        const { data: knownIps, error: ipLookupError } = await supabase.from("ips").select("id,name,display_name,is_active");
        if (ipLookupError) throw ipLookupError;
        const normalizedName = customIpName.toLocaleLowerCase();
        const existingIp = knownIps.find((ip) => ip.name.toLocaleLowerCase() === normalizedName || ip.display_name.toLocaleLowerCase() === normalizedName);
        if (existingIp) {
          primaryIpId = existingIp.id;
          if (!existingIp.is_active) {
            const { error: reactivateError } = await supabase.from("ips").update({ is_active: true }).eq("id", existingIp.id);
            if (reactivateError) throw reactivateError;
          }
        } else {
          const slugBase = customIpName.normalize("NFKD").toLowerCase().replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "custom";
          const { data: newIp, error: ipInsertError } = await supabase.from("ips").insert({
            name: customIpName,
            display_name: customIpName,
            slug: `${slugBase}-${crypto.randomUUID().slice(0, 8)}`,
            sort_order: 999,
            is_active: true,
          }).select("id").single();
          if (ipInsertError) throw ipInsertError;
          primaryIpId = newIp.id;
          createdIpId = newIp.id;
        }
      }
      ...
      if (createdIpId) await supabase.from("ips").delete().eq("id", createdIpId);
      if (storagePath) await supabase.storage.from("product-images").remove([storagePath]);
      const text = error instanceof Error ? error.message : String(error);
      setMessage(text.includes("duplicate") ? "SKU 或自定义 IP 已存在，请检查后再提交。" : `添加失败：${text}`);
    } finally {
      setSubmitting(false);
    }
  };
